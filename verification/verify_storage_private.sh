...
# Container-level remediation path: ensure publicAccess is private/None.
if [[ "$RESOURCE_ID" == *"/blobServices/default/containers/"* ]]; then
  public_access="$(az resource show --ids "$RESOURCE_ID" --query 'properties.publicAccess' -o tsv 2>/dev/null || true)"
  if [[ -z "$public_access" || "$public_access" == "None" ]]; then
    exit 0
  fi
  echo "storage container is still public for $RESOURCE_ID (publicAccess=${public_access})" >&2
  exit 1
fi
