...
class TestOPAEnrichment(unittest.TestCase):
    def test_enrichment_uses_effective_and_excepted_violations(self) -> None:
        ...
        opa_decisions = {
            ...
            "compliant": [{"resource_id": "azurerm_resource_group.c", "status": "COMPLIANT"}],
        }
        ...
        self.assertEqual(by_address["azurerm_storage_account.a"]["action_required"], "auto_remediate")
        self.assertEqual(by_address["azurerm_storage_account.a"]["custodian_policy"], "enforce-storage-tls")
        self.assertFalse(by_address["azurerm_storage_account.a"]["opa_excepted"])

        self.assertEqual(by_address["azurerm_network_security_group.b"]["severity"], "Info")
        self.assertEqual(by_address["azurerm_network_security_group.b"]["action_required"], "none")
        ...
        self.assertEqual(by_address["azurerm_virtual_network.d"]["action_required"], "manual_review")
    ...
    def test_fallback_to_raw_violations_when_effective_missing(self) -> None:
        drift_items = [{"address": "azurerm_sql_server.main", "type": "azurerm_sql_server"}]
        opa_decisions = {
            "violations": [
                {
